...
const (
	...
	allowedHostHeader = "so1.ai" // TODO: block other hosts
	routeNameCtxVar   = "routeName"
)

type healthCheckFunc func(echo.Context) error
...
func NewEcho(logLevel string) (*echo.Echo, error) {
	...
	// Configure logger
	{
		...
		lcfg.Skipper = func(c echo.Context) bool {
			return c.Request().RequestURI == healthCheckEndpoint || c.Request().RequestURI == "/" || c.Request().Method == http.MethodOptions
		}
	...
	e.Use(func(h echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			if !strings.HasSuffix(c.Request().Host, allowedHostHeader) && !strings.HasPrefix(c.Request().Host, "localhost") && c.Request().RequestURI != healthCheckEndpoint {
...
func (cc *Context) SetupHandlers(prefix string, healthCheckFunc *healthCheckFunc) (*echo.Group, func()) {
	e := cc.Echo()

	e.GET(healthCheckEndpoint, func(c echo.Context) error {
		var err error
		if healthCheckFunc != nil {
			err = (*healthCheckFunc)(c)
		}
		if err != nil {
			return err
		}
		return ApiResponse(c, http.StatusOK, map[string]interface{}{
			"ok":    true,
			"build": cc.BuildInfo(),
		})
	})
